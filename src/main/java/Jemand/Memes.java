...
public class Memes {
    ...
    private static final Font[] fonts = {
            ...
            getFontByName("CondensedExtraLight")};
    ...
    Memes(String template, BufferedImage... images) throws IOException {
        ...
        this.template = ImageIO.read(func.fileOfName("templates/" + template + ".png"));
    }
    ...
    public Optional<BufferedImage> getFinalMeme() {
        ...
        for (int i = 0; i < coordinates.length; i++) {
            ...
            try {
                images[i] = new ImageResizer(images[i], "image.png").resize(w, h).getBufferedImage();
    ...
    static BufferedImage[] convertStrings(String[] text, int[][] coordinates) {
        ...
        for (int i = 0; i < text.length && i < coordinates.length; i++) {
            String withoutWhite = func.WHITE_SPACE.matcher(text[i]).replaceAll("");
            if(func.isURL(withoutWhite)) {
                try {
            ...
            if(images[i] == null) {
                ...
                String[] ltxt = func.WHITE_SPACE.split(text[i]);
    ...
    static Font getFontByName(String name) {
        try {
            return Font.createFont(Font.TRUETYPE_FONT, func.fileOfName("templates/" + name + ".ttf"));
