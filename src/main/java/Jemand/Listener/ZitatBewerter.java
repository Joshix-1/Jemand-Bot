...
public class ZitatBewerter {
    static private final CustomEmoji upvote2 = func.getApi().getCustomEmojiById(609050310627033099L).get();
    static private final String upvote = upvote2.getMentionTag();
                    ...
                    if (event.getReaction().isPresent()) {
                        try {
                            if (event.getReaction().get().getEmoji().equalsEmoji(report)) {
                                ...
                                if (event.getReaction().get().getEmoji().equalsEmoji(upvote2)) {
                                ...
                                message.edit(embed.removeAllFields().setImage("").addField("\u200B", upvote + ": " + be.get(zid)));
