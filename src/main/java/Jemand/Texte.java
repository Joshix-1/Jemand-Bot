...
public class Texte {
    ...
    private void reloadSprache() {
        sprachliste = f.JsonFromFile("sprachliste.json");
    ...
    private void setSprache(String Sprache) {
        ...
        f.JsonToFile(sprachliste, "sprachliste.json");
        sprache = Sprache;
    }
    ...
    public Texte getString(String name) {
        reloadSprache();
        Map<String, Map<String, String>> map = f.JsonFromFile("texte.json");
        try {
        ...
        if(!func.StringBlank(output)) {
            ...
            output = output.replace("<RAND_NUM>", Integer.toString(f.getRandom(120, 1200)));
            if (user != null) {
                ...
                JSONObject prefixjs = f.JsonFromFile("prefix.json");
