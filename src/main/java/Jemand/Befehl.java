...
public class Befehl {
    ...
    private static final Pattern COMPILE = Pattern.compile(":regional_indicator_x:");
    private static final Pattern PATTERN = Pattern.compile(":o2:");
    ...
    public boolean FuehreAus () throws IOException, ExecutionException, InterruptedException {
        ...
        if(befehl.get().equalsIgnoreCase("restart") && func.getFileSeparator().equals("/")) {
            if(!func.userIsTrusted(user)) return false;
            Runtime.getRuntime().exec("chmod +x /usr/home/admin/Jemand/Jemand-1.0-SNAPSHOT/bin/Jemand");
            func.shutdown();
            System.exit(69);
            return true;
        }
        ...
        if ((befehl.get().equalsIgnoreCase("goq") || befehl.get().equalsIgnoreCase("game-of-quotes"))) {
            ...
            AtomicReference<int[]> zid0 = new AtomicReference<>(func.differentRandomInts(0, zitate.length, 6));

            embed.setTitle("Wähle ein Zitat:").removeAllFields();
            for (int j = 0; j < zid0.get().length; j++) {
                embed.addField(helpabc[j], func.LinkedEmbed(zitate[zid0.get()[j]]));
                ...
                if (event2.getUser().equals(user)) {
                    if (iz.get() == -1) {
                        if (event2.getEmoji().equalsEmoji(EmojiParser.parseToUnicode(":repeat:"))) {
                            ...
                            for (int i = 0; i < 6; i++) {
                                if (event2.getEmoji().equalsEmoji(helpabc[i])) {
                                    ...
                                    embed.removeAllFields().setTitle("Zitat:").setDescription(func.LinkedEmbed(zitate[iz.get()]) + "\n\n - Schreibe einen witzigen Autoren in den Chat, oder wähle einen aus den unteren.\n\u200B");
                                    for (int j = 0; j < zid0.get().length; j++) {
                                        embed.addField(helpabc[j], func.LinkedEmbed(namen[zid0.get()[j]]));
                                    }
                                    message.edit(embed);
                                    event.getChannel().addMessageCreateListener(event3 -> {
                                        if (event3.getMessageAuthor().isUser() && in.get() == -1) {
                                            if (event3.getMessageAuthor().asUser().get().equals(user)) {
                                                if (func.isURL(event3.getMessage().getContent().replaceAll(" ", "")) || event3.getMessageContent().contains("://") || event3.getMessageContent().contains("!") || event3.getMessageContent().contains("?") || (event3.getMessageContent().contains(".") && !event3.getMessageContent().contains(" "))) {
                                                    event3.getMessage().addReaction("crossmark:572860377977847982");
                                                    ...
                                                    if (func.goq_replace(func.readtextoffile("namen.txt").toLowerCase()).contains(func.goq_replace(mc.toLowerCase())) && !(mc.endsWith("\"") && mc.startsWith("\""))) {
                                                        for (int j = 0; j < namen.length; j++) {
                                                            if (func.goq_replace(namen[j]).contains(func.goq_replace(mc))) {
                                                                in.set(j);
                                                                break;
                                                            }
                                                        }
                        ...
                        if (in.get() == -1) {
                            if (event2.getEmoji().equalsEmoji(EmojiParser.parseToUnicode(":repeat:"))) {
                                zid0.set(func.differentRandomInts(0, namen.length, 6));
                                embed.removeAllFields().setTitle("Zitat:").setDescription(zitate[iz.get()] + "\n\n - Schreibe einen witzigen Autoren in den Chat, oder wähle einen aus den unteren.\n\u200B");
                                for (int i = 0; i < zid0.get().length; i++) {
                                    embed.addField(helpabc[i], func.LinkedEmbed(namen[zid0.get()[i]]));
                                }
                                message.edit(embed);
                                event2.removeReaction();
        ...
        if (befehl.get().equalsIgnoreCase("save-secure") || befehl.get().equalsIgnoreCase("ss")) {
            ...
            if (befehl.get().equalsIgnoreCase("report")) {
                if (subtext1.get().isEmpty()) {
                    ...
                    event.getApi().getOwner().get().sendMessage(embed.setDescription(subtext1.get()).setTitle("Report (Server: " + server.getName()).addField( "UserID:", user.getIdAsString()));
                    return true;
                }
            }
