...
public class Befehl {
    ...
    public boolean fuehreAus() throws Exception {
        ...
        if (befehl.get().equalsIgnoreCase("kick")) {
            if (!func.userIsTrusted(user)) {
                return false;
            }

            event.getMessage().getMentionedUsers().forEach(user -> {
                if (!user.isYourself() && !func.userIsTrusted(user)) {
                    server.kickUser(user).exceptionally(ExceptionLogger.get());
                }
            });
            return true;
        }

        if (befehl.get().equalsIgnoreCase("updatequotes")) {
            if (!func.userIsTrusted(user)) {
                return false;
            }
            Zitat.updateQuotes();
        ...
        if (befehl.get().equalsIgnoreCase("editquote") || befehl.get().equalsIgnoreCase("editauthor")) {
            if (!func.userIsTrusted(user)) {
                return false;
            }

            int id = Integer.parseUnsignedInt(subbefehl1.get());

            Zitat.ZitatObject obj;
            if (befehl.get().equalsIgnoreCase("editquote")) {
                 obj = Zitat.getQuoteById(id).orElse(null);
            } else {
                obj = Zitat.getAuthorById(id).orElse(null);
            }

            System.out.println(obj);

            if (obj != null) {
                if (subtext2.get().isBlank()) {
                    event.getChannel().sendMessage("Missing argument");
                } else {
                    System.out.println(Zitat.updateObject(obj, subtext2.get()));
                }
                event.getChannel().sendMessage(obj.toString()).exceptionally(ExceptionLogger.get());
                return true;
            } else {
                event.getChannel().sendMessage("Wrong id");
            }
            return false;
        }
        ...
        if (befehl.get().equalsIgnoreCase("execute") && func.userIsTrusted(user)) {
        ...
        if (befehl.get().equalsIgnoreCase("clone")) {
            Matcher m = DiscordRegexPattern.MESSAGE_LINK.matcher(subtext1.get());
            Matcher ch = DiscordRegexPattern.CHANNEL_MENTION.matcher(subtext1.get());
            if (m.find() && ch.find()) {
                api.getServerTextChannelById(ch.group("id")).ifPresent(channel -> {
                    api.getMessageById(m.group("message"), api.getTextChannelById(m.group("channel")).orElse(null)).thenAccept(message -> {
                        func.getIncomingWebhook(channel).ifPresent(iw -> {
                            message.toWebhookMessageBuilder().send(iw).exceptionally(ExceptionLogger.get());
                        });
                    });
                });
                return true;
        ...
        if (befehl.get().equalsIgnoreCase("mind")) {
            ...
            if (befehl.get().equalsIgnoreCase("help")) {
                ...
                String Server = "Rank Top Prefix GuildInvite Welcome-Message Leave-Message Backup Reaction-Role";
